...
ESP8266_ILI9341::ESP8266_ILI9341(int8_t cs, int8_t dc, int8_t mosi,
...
ESP8266_ILI9341::ESP8266_ILI9341(int8_t dc, int8_t rst) : Adafruit_GFX(ILI9341_TFTWIDTH, ILI9341_TFTHEIGHT) {
  _dc   = dc;
  _rst  = rst;
  hwSPI = true;
#ifdef ESP8266
...
#else
  ESP8266_ILI9341::ESP8266_ILI9341(int8_t cs, int8_t dc, int8_t rst) : Adafruit_GFX(ILI9341_TFTWIDTH, ILI9341_TFTHEIGHT) {
    _cs   = cs;
    _dc   = dc;
    _rst  = rst;
  #ifdef ESP8266
...
#ifdef ESP8266
void ESP8266_ILI9341::spiwrite16(uint16_t c) {
    SPI.write16(c, true);
}
#endif

void ESP8266_ILI9341::spiwrite(uint8_t c) {
...
void ESP8266_ILI9341::spiwriteBytes(uint8_t * data, uint32_t size) {
#ifdef ESP8266
...
void ESP8266_ILI9341::spiwritePattern(uint8_t * data, uint8_t size, uint32_t repeat) {
#ifdef ESP8266
...
inline void ESP8266_ILI9341::spiCsLow(void) {
...
inline void ESP8266_ILI9341::spiCsHigh(void) {
#ifdef ILI9341_USE_DIGITAL_WRITE
...
inline void ESP8266_ILI9341::spiDcLow(void){
#ifdef ILI9341_USE_DIGITAL_WRITE
...
inline void ESP8266_ILI9341::spiDcHigh(void) {
#ifdef ILI9341_USE_DIGITAL_WRITE
...
void ESP8266_ILI9341::writecommand(uint8_t c) {
    spiDcLow();
    spiCsLow();
...
void ESP8266_ILI9341::writedata(uint8_t c) {
    spiDcHigh();
    spiCsLow();
...
void ESP8266_ILI9341::writedata(uint8_t * data, uint8_t size) {
    spiDcHigh();
    spiCsLow();
...
void ESP8266_ILI9341::writeCmdData(uint8_t cmd, uint8_t * data, uint8_t size) {
    spiDcLow();
    spiCsLow();
...
uint16_t ESP8266_ILI9341::getHeight(void) {
    return _height;
}

uint16_t ESP8266_ILI9341::getWidth(void){
    return _width;
}
...
void ESP8266_ILI9341::commandList(uint8_t *addr) {
...
void ESP8266_ILI9341::begin(void) {
  if (_rst > NOT_A_PIN) {
    pinMode(_rst, OUTPUT);
...
void ESP8266_ILI9341::area_update_start(uint32_t x, uint32_t y, uint32_t w, uint32_t h) {
    spiCsLow();
    setAddrWindow_(x, y, x + w - 1, y + h - 1);
}

void ESP8266_ILI9341::area_update_data(uint8_t *data, uint32_t pixel){
    spiwriteBytes(&data[0], (pixel*2));
}

void ESP8266_ILI9341::area_update_end(void){
    spiCsHigh();
}

void ESP8266_ILI9341::setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
    spiCsLow();
    setAddrWindow_(x0, y0, x1, y1);
    spiCsHigh();
}

void ESP8266_ILI9341::setAddrWindow_(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
...
void ESP8266_ILI9341::pushColor(uint16_t color) {
  if (hwSPI) spi_begin();
...
void ESP8266_ILI9341::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
...
void ESP8266_ILI9341::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
...
// fill a rectangle
void ESP8266_ILI9341::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
...
uint8_t ESP8266_ILI9341::spiread(void) {
  uint8_t r = 0;
...
uint8_t ESP8266_ILI9341::readdata(void) {
    if(hwSPI) spi_begin();
    spiCsLow();
    spiDcLow();
    uint8_t r = spiread();
...
uint8_t ESP8266_ILI9341::readcommand8(uint8_t c, uint8_t index) {
    if(hwSPI) spi_begin();
...
/*
 ...
 uint16_t ESP8266_ILI9341::readcommand16(uint8_t c) {
 digitalWrite(_dc, LOW);
 if (_cs)
 digitalWrite(_cs, LOW);
 ...
 uint32_t ESP8266_ILI9341::readcommand32(uint8_t c) {
 digitalWrite(_dc, LOW);
 if (_cs)
 digitalWrite(_cs, LOW);
 spiwrite(c);
